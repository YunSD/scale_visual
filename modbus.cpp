...
/* Exported version */
const unsigned int libmodbus_version_major = 123;
const unsigned int libmodbus_version_minor = 123;
const unsigned int libmodbus_version_micro = 123;
    ...
    modbus_mapping_t* mb_mapping)
{
    ...
    case MODBUS_FC_MASK_WRITE_REGISTER: {
        ...
        if (mapping_address < 0 || mapping_address >= mb_mapping->nb_registers) {
            ...
            uint16_t _data = mb_mapping->tab_registers[mapping_address];
            uint16_t _and = (req[offset + 3] << 8) + req[offset + 4];
            uint16_t _or = (req[offset + 5] << 8) + req[offset + 6];

            _data = (_data& _and) | (_or &(~_and));
            mb_mapping->tab_registers[mapping_address] = _data;
            memcpy(rsp, req, req_length);
            rsp_length = req_length;
        }
    } break;
